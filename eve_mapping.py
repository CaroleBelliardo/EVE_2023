...
parser.add_argument('-pb', '--pre_bed', type=str, help='Eve annotation file (.bed file) \n')
...
def errorArgsBed(bed_init,bed_fin):
    ...
    try: ## Bed annotation EVEs
        ...
        b_bedCreate=True ## flag le fichier bed ( avec flanq n'existe pas ) mais le fichier pre_bed ( sans flanq ) existe 
    except IOError :
...
# contig size
def f_LenContig (lenC,genome,pre_bed) :
    LenContig={}
    ...
    os.system('cut -f 1 ' +pre_bed+' > '+pre_bed+'.cut ; grep -w -f '+pre_bed+'.cut '+lenC+' >'+ lenC+'.cut' )        
...
#########################                                          ***  TABLES    ***                                                  ###########################
##################################################################################################################################################################
##Supprimer l'entete et installer samtools

###########################
##       EACH  EVE       ##
###########################
b_bedCreate=errorArgsBed(args.pre_bed,args.pre_bed+'.flanc')
if b_bedCreate == True : 
	LenContig = f_LenContig(args.lenC,args.genome,args.pre_bed)
...
PreBed.close()
Bed.close()
args.bed=args.pre_bed+'.flanc'
f_bamToTable(args.sB,args.lB,args.bed,args.genome,args.output)


# #############################                                            PLOTS                                                       #############################
### *** fichiers Necessaires pour plots # ## ET
# def f_bedIntersect(te,bed):
#     # 'cat '+te+'| awk -F" " '{print $5 "\t" $6 "\t" $7 "\t" $9 "\t" $11}'' 
#     # os.system('bedtools intersect -a '+te+'  -b '+bed+' > '+te+'.flanc')
#     print('bedtools intersect -a '+te+'  -b '+bed+' > '+te+'.flanc')
#     # return (te+'.flanc')
# 
## bedcov
##def f_bedCov(bam,bed):
  ##  os.system('samtools bedcov '+bed+' '+bam+' > ../bedcov/'+bam+'.cov')
    ##print('-----\nbedcov ok !! \n file :: ../bedcov/'+bam+'.cov')
    ##return (bam+'.cov')
# print('path :' + args.TE)
# teFile=f_bedIntersect(args.TE,args.bed) ### recup fonction R avant et apres et fichier bed juste flanq
# print('***stop')
#sbCov=f_bedCov(args.sB,args.pre_bed)
#lbCov=f_bedCov(args.lB,args.pre_bed)
